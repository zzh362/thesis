...
\subsection{自适应特征扰动}
特征扰动是半监督学习的关键策略之一。本文采用虚拟对抗训练（VAT\cite{miyato2018virtual}）生成对抗噪声，而非随机噪声，目的是最大化扰动前后预测的差异。如图 \ref{fig_sspsd} 左下部分所示，给定潜在特征\(\textbf{z}\)，通过梯度引导生成对抗噪声\(\textbf{r}\)，使\((D_\theta(\textbf{z}+\textbf{r}) - D_\theta(\textbf{z}))\)最大化，以干扰网络预测。要达到理想的扰动效果，\(D_\theta\)的模型质量至关重要，因其决定了梯度的搜索方向。
...
\subsection{一致性约束}
构建无标签数据的一致性约束时，直接计算学生与教师模型预测的MSE损失可能强制错误预测保持一致，进而误导网络学习。此外，对特征图\(\mathbb{R}\)中所有网格单元均等加权也并非最优策略。为此，如图 \ref{fig_sspsd} 右下部分所示，本文引入置信度引导掩码（CGM），为不同网格单元分配非均衡注意力，并对低置信度区域进行掩码（Mask）处理。
